...
const getLevelStyles = (level: CourseLevel) => {
  switch (level) {
    case "BEGINNER": return "text-emerald-600 border-emerald-200 bg-emerald-50 dark:text-emerald-400 dark:border-emerald-900/50 dark:bg-emerald-950/20";
    case "INTERMEDIATE": return "text-amber-600 border-amber-200 bg-amber-50 dark:text-amber-400 dark:border-amber-900/50 dark:bg-amber-950/20";
    case "ADVANCED": return "text-rose-600 border-rose-200 bg-rose-50 dark:text-rose-400 dark:border-rose-900/50 dark:bg-rose-950/20";
    default: return "text-zinc-500 border-zinc-200 bg-zinc-50";
  }
};
  ...
  return (
    ...
    >
      ...
      <div className="col-span-12 md:col-span-2 flex md:flex-col items-center md:items-start justify-between md:justify-start gap-3">
        ...
        <span className={`text-[9px] border px-2 py-0.5 font-bold transition-colors ${getLevelStyles(course.level)}`}>
          {course.level}
        </span>
      </div>
      ...
      <div className="col-span-12 md:col-span-3 flex flex-row md:flex-col items-center md:items-end justify-between pt-4 md:pt-0 border-t md:border-t-0 border-zinc-100 dark:border-zinc-900">
        <div className="md:text-right">
          <span className="block text-[10px] text-zinc-400 font-bold">Duration</span>
          <span className="text-base font-bold tabular-nums tracking-normal">{course.duration}</span>
        </div>
...
function CourseButton({ status }: { status: Course["status"] }) {
  ...
  return (
    <div className={`border-2 px-6 py-2 bg-transparent transition-none text-xs font-bold tracking-widest whitespace-nowrap ${
        status === "completed" 
        ? "border-emerald-500 text-emerald-600 dark:text-emerald-400 group-hover:bg-emerald-500 group-hover:text-white" 
